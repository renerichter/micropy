'''
   ~~~~ Package description ~~~~
    The Filters are build such that they assume to receive an nD-stack, but they only operate in a 2D-manner (meaning: interpreting the stack as a (n-2)D series of 2D-images). Further, they assume that the 1st two dimensions (0,1) are the image-dimensions. The others are just for stacking.
...
def cf_vollathF4_symmetric(im, im_out=True):
    ...
    '''
    impix = 1.0/get_nbrpixel(im, dim=[0, 1])
    im_res = cf_vollathF4_symmetric_corr(im, keep_size=True)
    res = impix * (np.sum(np.abs(np.sum(im_res, axis=(1, 2))), axis=0))
    if im_out:
        return res, im_res
    else:
        return res
...
def cf_vollathF5(im, im_out=True):
    ...
    res = impix*(np.sum(im_res, axis=(0, 1)) - 1.0 /
                 impix * np.sum(im_res, axis=(0, 1))**2)
    if im_out:
        return res, im_res
    else:
        return res
...
def spf_dct_normalized_shannon_entropy(im, r0=100):
    ...
    im_res = nip.image(dct2(im, forward=True, axes=[0, 1]))
...
def stf_basic(im, printout=False):
    ...
    '''
    im_res = list()
    im_res.append(np.max(im, axis=(0, 1)))
    im_res.append(np.min(im, axis=(0, 1)))
    im_res.append(np.mean(im, axis=(0, 1)))
    im_res.append(np.median(im, axis=(0, 1)))
    im_res.append(np.var(im, axis=(0, 1)))
...
def stf_kurtosis(im):
    ...
    res = np.mean((im-np.mean(im, axis=(0, 1)))**4) / \
        np.var(im, axis=(0, 1))**2
    return res
...
def stf_diffim_kurtosis(im):
    ...
    '''
    return stf_kurtosis(im[1:, 1:] - im[:-1, :-1])
...
# -------------------------------------------------------------------------
#
def dct2(im, forward=True, axes=[0, 1]):
    '''
...
def euler_forward_1d(im, dim=0, dx=1):
    ...
    # rotate image
    # does transpose change the view or the array?
